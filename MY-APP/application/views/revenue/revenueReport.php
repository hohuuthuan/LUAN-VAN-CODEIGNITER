	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		timeTypeSelect.addEventListener("change", function() {
			...
			if (selectedValue === "day") {
				...
				startDateInput.name = "start_year";
				endDateInput.name = "end_year";
...
<script>
	...
	<?php if (!empty($profits)): ?>
		...
		<?php elseif ($timeType === 'year'): ?>
			chartData.labels = [
				<?php foreach ($profits['yearlyTotals'] as $year => $totals): ?> "<?= htmlspecialchars($year) ?>",
				<?php endforeach; ?>
			];
			chartData.revenue = [
				<?php foreach ($profits['yearlyTotals'] as $totals): ?>
					<?= $totals['Total_Revenue'] ?>,
				<?php endforeach; ?>
			];
			chartData.cost = [
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		const revenueChart = new Chart(ctx, {
			type: 'bar',
			data: {
				labels: chartData.labels, // Trục X: Ngày/Tháng/Năm
				datasets: [{
						label: 'Doanh thu',
			...
			options: {
				...
				scales: {
					x: {
						title: {
							display: true,
							text: timeType === 'day' ? 'Thời gian (Ngày)' : (timeType === 'month' ? 'Thời gian (Tháng)' : 'Thời gian (Năm)')
						}
					},
					y: {
						beginAtZero: true,
